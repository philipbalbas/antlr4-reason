/*
 * GRAKN.AI - THE KNOWLEDGE GRAPH Copyright (C) 2019 Grakn Labs Ltd
 * 
...
// Graql end-of-file (aka. end-of-string) query parser Needed by Graql's Parser to ensure that it
...
query:
	query_define
	| query_undefine // define / undefine types from schema
	| query_insert
	| query_delete // insert / delete data from graph
	| query_get
	| query_get_aggregate // read data from graph (OLTP)
	| query_get_group
	| query_get_group_agg
...
query_insert:
	MATCH pattern+ INSERT statement_instance+
	| INSERT statement_instance+;

query_delete:
	MATCH pattern+ DELETE variables filters; // GET QUERY followed by aggregate fn
query_get:
	MATCH pattern+ GET variables filters; // GET QUERY followed by group fn, and
...
query_get_group_agg:
	query_get function_group function_aggregate;
...
function_aggregate:
	function_method VAR_? ';'; // method and, optionally, a variable
function_method:
	COUNT
	| MAX
	| MEAN
...
pattern_statement: statement_type | statement_instance;
...
statement_relation:
	...
	| VAR_? relation ';';
statement_attribute:
	VAR_? operation ISA_ type (',' attributes)? ';'
...
// assertion between an Attribute and its owner (any Thing) ATTRIBUTE CONSTRUCT
// =========================================================

attributes: attribute ( ',' attribute)*;
attribute:
	HAS type_label (VAR_ | operation) via?; // Attribute ownership by variable or a
// predicate, and the "via" Relation ATTRIBUTE OPERATION CONSTRUCTS
// ==============================================
...
conditions_value:
	compute_method input_value (',' input_value)* ';';
conditions_central:
	CENTRALITY input_central (',' input_central)* ';';
conditions_cluster:
	CLUSTER input_cluster (',' input_cluster)* ';';
...
compute_direction:
	FROM ID_ // an instance to start the compute from
	| TO ID_; // an instance to end the compute at
compute_target:
	OF type_labels; // type(s) of instances to apply compute
compute_scope:
	IN type_labels; // type(s) to scope compute visibility
compute_config:
...
compute_arg:
	MIN_K '=' INTEGER_ // a single argument for min-k=INTEGER
...
// UNRESERVED KEYWORDS ========================================================= Most of Graql
...
// GRAQL INPUT TOKEN PATTERNS All token names must end with an underscore ('_')
